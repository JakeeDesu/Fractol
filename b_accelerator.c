...
/*   Updated: 2019/05/19 14:23:52 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	b_exec(cl_command_queue *c_q, cl_program *program, cl_mem *mem, t_o *o)
{
	int			size;
	cl_kernel	kernel;
	...
	size = o->h * o->w;
	kernel = clCreateKernel(*program, "burning_ship_gen", NULL);
...
void	b_parallel(cl_device_id *device_id, cl_context *c, t_o *o, t_complex z)
{
	t_set_1				*data;
	...
	mem = b_creat_mem(c, o);
	bcpm(&command_queue, mem, o, z);
	program = read_kernel("burning_ship_gen_kernel.cl", c);
	clBuildProgram(program, 1, device_id, NULL, NULL, NULL);
	b_exec(&command_queue, &program, mem, o);
...
void	b_accelerator(t_complex z, t_o *o)
{
	...
	b_parallel(&device_id, &context, o, z);
