...
/*   Updated: 2019/05/19 14:28:07 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	d_exec(cl_command_queue *c_q, cl_program *program, cl_mem *mem, t_o *o)
{
	int			size;
	cl_kernel	kernel;
	...
	size = o->h * o->w;
	kernel = clCreateKernel(*program, "drop_gen", NULL);
...
void	d_parallel(cl_device_id *device_id, cl_context *c, t_o *o, t_complex z)
{
	t_set_1				*data;
	...
	program = read_kernel("drop_gen_kernel.cl", c);
	clBuildProgram(program, 1, device_id, NULL, NULL, NULL);
	d_exec(&command_queue, &program, mem, o);
...
void	d_accelerator(t_complex z, t_o *o)
{
	...
	d_parallel(&device_id, &context, o, z);
