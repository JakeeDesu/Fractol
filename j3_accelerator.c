...
/*   Updated: 2019/05/19 18:14:03 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	exec3(cl_command_queue *c_q, cl_program *program, cl_mem *mem, t_o *o)
{
	int			size;
	cl_kernel	kernel;
	...
	size = o->h * o->w;
	kernel = clCreateKernel(*program, "julia3_gen", NULL);
...
void	parallel3(cl_device_id *device_id, cl_context *c, t_o *o, t_complex *cz)
{
	t_set_1				*data;
	...
	program = read_kernel("julia3_gen_kernel.cl", c);
	clBuildProgram(program, 1, device_id, NULL, NULL, NULL);
	exec3(&command_queue, &program, mem, o);
...
void	j3_accelerator(t_complex c, t_complex z, t_o *o)
{
	...
	parallel3(&device_id, &context, o, cz);
