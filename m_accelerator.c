...
/*   Updated: 2019/05/19 14:18:57 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	m_exec(cl_command_queue *c_q, cl_program *program, cl_mem *mem, t_o *o)
{
	int			size;
	cl_kernel	kernel;
	...
	size = o->h * o->w;
	kernel = clCreateKernel(*program, "mandlbrot_gen", NULL);
...
void	m_parallel(cl_device_id *device_id, cl_context *c, t_o *o, t_complex z)
{
	t_set_1				*data;
	...
	program = read_kernel("mandlbrot_gen_kernel.cl", c);
	clBuildProgram(program, 1, device_id, NULL, NULL, NULL);
	m_exec(&command_queue, &program, mem, o);
...
void	m_accelerator(t_complex z, t_o *o)
{
	...
	m_parallel(&device_id, &context, o, z);
