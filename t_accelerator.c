...
/*   Updated: 2019/05/19 16:26:39 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractal.h"

void	t_exec(cl_command_queue *c_q, cl_program *program, cl_mem *mem, t_o *o)
{
	int			size;
	cl_kernel	kernel;
	...
	size = o->h * o->w;
	kernel = clCreateKernel(*program, "tricorn_gen", NULL);
...
void	t_parallel(cl_device_id *device_id, cl_context *c, t_o *o, t_complex z)
{
	t_set_1				*data;
	...
	program = read_kernel("tricorn_gen_kernel.cl", c);
	clBuildProgram(program, 1, device_id, NULL, NULL, NULL);
	t_exec(&command_queue, &program, mem, o);
...
void	t_accelerator(t_complex z, t_o *o)
{
	...
	t_parallel(&device_id, &context, o, z);
